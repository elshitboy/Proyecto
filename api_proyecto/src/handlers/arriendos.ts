...
export const crearArriendo = async (request: Request, response: Response) => {
    try {
        ...
        //ahora hacemos la creacion del arriendo 
        const nuevoArriendo = await Arriendo.create({
            patente_vehiculo,
            tipo_vehiculo,
            rut_cliente,
            nombre_cliente,
            fecha_inicio: new Date(), //fecha de inicio es la fecha actual para cada arriendo
            fecha_termino: null //la fecha de termino es null al inicio ya que el arriendo no ha terminado
        })
...
export const devolucionVehiculo = async (request: Request, response: Response) => {
    try {
        ...
        arriendo.fechatermino = new Date() //fecha de termino es la fecha actual
